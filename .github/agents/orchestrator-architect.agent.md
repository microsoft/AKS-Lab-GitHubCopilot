...
5. `docker-compose.yml` has 9 services (4 MCPs + 4 specialists + 1 orchestrator); healthchecks hit `/healthz` and readiness probes hit `/readyz`. `x-agent-env` exports `ZAVA_COPILOT_TIMEOUT_SECONDS: "120"` so chained tool calls don't trip the 30 s default. `GITHUB_TOKEN` is referenced via `${GITHUB_TOKEN:?...}` — required even for `down`/`ps`.
